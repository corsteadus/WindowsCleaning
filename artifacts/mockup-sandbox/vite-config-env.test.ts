...
describe("mockup Vite build/runtime environment resolution", () => {
  ...
  it("preserves injected PORT and BASE_PATH values", () => {
    assert.equal(resolveVitePort("8081", "serve"), 8081);
    assert.equal(resolveViteBasePath("/__mockup", "serve"), "/__mockup");
  });
